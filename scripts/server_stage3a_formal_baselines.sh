#!/usr/bin/env bash
# 阶段3A：在正式数据上并行训练图基线和向量拓扑条件基线，仅评价 validation 拓扑。

set -euo pipefail

data_path="${1:-data/ieee33_feasible_T300_S500_seed2026.npz}"
epochs="${FORMAL_EPOCHS:-40}"
train_batch="${FORMAL_BATCH_SIZE:-1024}"
sample_batch="${FORMAL_SAMPLE_BATCH_SIZE:-256}"
samples_per_topology="${FORMAL_SAMPLES_PER_TOPOLOGY:-100}"
...
run_experiment() {
  local model_type="$1"
  local device="$2"
  local output_dir="outputs/formal_${model_type}_linear_T1000_E${epochs}_seed2026"
...
echo "Launching graph baseline on cuda:0 and vector baseline on cuda:1."
run_experiment graph cuda:0 > logs/formal_graph_baseline.log 2>&1 &
graph_pid=$!
run_experiment vector cuda:1 > logs/formal_vector_baseline.log 2>&1 &
vector_pid=$!
...
if [[ "${status}" -ne 0 ]]; then
  echo "At least one formal baseline failed. Inspect model-specific logs." >&2
  exit 1
fi
...
summary = []
for model_type in ("graph", "vector"):
    output_dir = Path(f"outputs/formal_{model_type}_linear_T1000_E${epochs}_seed2026")
    ...
    summary.append(
        {
            ...
            "validation_topology_count": metrics["unique_generated_topologies"],
...
destination = Path("docs/formal_validation_baselines_summary.json")
destination.write_text(json.dumps(summary, indent=2), encoding="utf-8")
print(f"Formal validation baseline summary written to: {destination}")
PY

echo "Stage 3A complete. Test topologies were not sampled."
